...
# Remove stale transformers_4573.egg-info directory to avoid https://github.com/pypa/pip/issues/5466
stale_egg_info = Path(__file__).parent / "transformers_4573.egg-info"
if stale_egg_info.exists():
    print(
        f"Warning: {stale_egg_info} exists.\n\n"
        "If you recently updated transformers_4573 to 3.0 or later, this is expected,\n"
        "but it may prevent transformers_4573 from installing in editable mode.\n\n"
...
# 2. once modified, run: `make deps_table_update` to update src/transformers_4573/dependency_versions_table.py
_deps = [
    "Pillow>=10.0.1,<=15.0",
    "accelerate>=1.1.0",
    "av",
    ...
    "ruff==0.13.1",
    # `sacrebleu` not used in `transformers_4573`. However, it is needed in several tests, when a test calls
...
# since we save this data in src/transformers_4573/dependency_versions_table.py it can be easily accessed from
# anywhere. If you need to quickly access the data from this table in a shell, you can do so easily with:
#
# python -c 'import sys; from transformers_4573.dependency_versions_table import deps; \
# print(" ".join([ deps[x] for x in sys.argv[1:]]))' tokenizers datasets
#
# Just pass the desired package names to that script as it's shown with 2 packages above.
#
# If transformers_4573 is not yet installed and the work is done from the cloned repo remember to add `PYTHONPATH=src` to the script above
#
# You can then feed this for example to `pip`:
#
# pip install -U $(python -c 'import sys; from transformers_4573.dependency_versions_table import deps; \
# print(" ".join([deps[x] for x in sys.argv[1:]]))' tokenizers datasets)
#
...
class DepsTableUpdateCommand(Command):
    ...
    user_options = [
        ...
        ("dep-table-update", None, "updates src/transformers_4573/dependency_versions_table.py"),
    ]
    ...
    def run(self):
        ...
        target = "src/transformers_4573/dependency_versions_table.py"
...
setup(
    ...
    author="The Hugging Face team (past and future) with the help of all our contributors (https://github.com/huggingface/transformers_4573/graphs/contributors)",
    author_email="transformers_4573@huggingface.co",
    description="transformers_4573: the model-definition framework for state-of-the-art machine learning models in text, vision, audio, and multimodal models, for both inference and training.",
    ...
    url="https://github.com/huggingface/transformers_4573",
    package_dir={"": "src"},
    ...
    entry_points={"console_scripts": ["transformers_4573=transformers_4573.cli.transformers_4573:main"]},
